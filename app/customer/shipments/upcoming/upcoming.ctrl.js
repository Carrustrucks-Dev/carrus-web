angular.module('carrus').controller('upcomingctrl',['$scope','$http','$cookieStore',function($scope,$http,$cookieStore) {
    ...
    console.log(" ongoing");
    function initTable() {
        $http.get("http://52.25.204.93:8080/api/v1/shipper/getConfirmed?limit=6&skip=" + $scope.skip + "&", {headers: {'Authorization': $cookieStore.get('obj').accessToken}})
            .success(function (data, status) {
                var ConfirmedList = [];
                console.log(data);
                var list = data.data;
                list.forEach(function (column) {
                    ...
                    ConfirmedList.push(h);

                    // console.log($scope.ConfirmedList);
                });
                $scope.ConfirmedList = ConfirmedList;
            })
