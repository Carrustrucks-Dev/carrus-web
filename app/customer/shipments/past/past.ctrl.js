angular.module('carrus').controller('pastctrl',['$scope','$http','$cookieStore',function($scope,$http,$cookieStore){
    ...
    function initTable() {
        $http.get("http://52.25.204.93:8080/api/v1/shipper/getPast?limit=6&skip=" + $scope.skip + "&", {headers: {'Authorization': $cookieStore.get('obj').accessToken}})
            .success(function (data, status) {
                var pastList = [];
                console.log(data);
                var list = data.data;
                list.forEach(function (column) {
                    ...
                    pastList.push(h);
                    $scope.pastList = pastList;
                    console.log($scope.pastList);
                });
            })
